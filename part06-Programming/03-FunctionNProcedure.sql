-- Hàm : 1 nhóm câu lệnh dc đặt 1 tên,nhóm lệnh này làm 1 việc gì đó, để dùng lại re-use
...
-- R.DBMS ( Cơ sở dựa trên Relation/Table) ta có 2 loại hàm y chang 
-- Procedure tương đưng với VOID 
-- Function ~~~~ Return 
...
-- gọi hàm f()
-- int x,y 
-- f(&x,&y) , khi trong hàm f mà làm gì với x và y thì x , y ở bên ngoài bị ảnh hưởng luôn 
-- hàm thay đổi 2 giá trị x y ở bên ngoài, void mà lại đưa data ra ngoài qua con trỏ, truyền tham chiếu
-- pass by reference
...
CREATE PROC PR_C2F
@cDegree float -- @ là đại diện cho giá trị 
AS
BEGIN
